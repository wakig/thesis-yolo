...
while True:
    ...
    if len(results) > 0:
        ...
        #     cx = x_min + box_width/2
        #     cy = y_min + box_height/2
        ...
        for tracker in trackers:
            ...
            for i in detections:
                ...
                cx = x_min + box_width/2
                cy = y_min + box_height/2
                detection_box = np.array([x_min, y_min, x_min+box_width, y_min+box_height])
                predicted_box = np.array([tracker.x[0, 0]-box_width/2, tracker.x[1, 0]-box_height/2, tracker.x[0, 0]+box_width/2, tracker.x[1, 0]+box_height/2])
                iou = compute_iou(detection_box, predicted_box)
                
                # Keep track of the detection with the highest IoU
                if iou > best_iou:
                    best_iou = iou
                    best_detection = bounding_boxes[i]
            ...
            if best_detection is not None and best_iou > IOU_THRESHOLD:
                x_min, y_min = best_detection[0], best_detection[1]
                box_width, box_height = best_detection[2], best_detection[3]
                measurement = np.array([x_min + box_width/2, y_min + box_height/2])
                tracker.update(measurement)

                cv2.circle(frame, (int(tracker.x[0][0]), int(tracker.x[1][0])),
                10,
                (255,0,0), cv2.FILLED)
                start_x = tracker.x[0][0]
                start_y = tracker.x[1][0]
                dx = tracker.x[2][0]
                dy = tracker.x[3][0]
                end_x = start_x + dx
                end_y = start_y + dy
                ...
                for i in detections:
                    if best_detection == bounding_boxes[i]:
                        x_min, y_min = bounding_boxes[i][0], bounding_boxes[i][1]
                        box_width, box_height = bounding_boxes[i][2], bounding_boxes[i][3]
                        # Drawing bounding box on the original current frame
                        cv2.rectangle(frame, (x_min, y_min),
